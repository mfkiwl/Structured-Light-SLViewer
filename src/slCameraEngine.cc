...
SLCameraEngine::SLCameraEngine(QObject *parent)
    : __camera(nullptr), __vtkProcessEngine(nullptr), __isStartDetect(false),
      __isDetected(false), __isAlignedMode(false), __isOfflineMode(false) {}
...
void SLCameraEngine::configCamera(const QString jsonPath,
...
bool SLCameraEngine::connect() { return __camera->connect(); }

bool SLCameraEngine::disConnect() { return __camera->disConnect(); }

bool SLCameraEngine::isConnect() { return __camera->isConnect(); }
...
bool SLCameraEngine::capture() {
    ...
    if (__isOfflineMode) {
        isSucess =
            offLineCapture("../out/offlineData/L", "../out/offlineData/R");
    } else {
    ...
    cv::Mat depthColor =
        (__frameData.__depthMap - minVal) / (maxVal - minVal) * 255;
    depthColor.convertTo(depthColor, CV_8UC1);
    cv::applyColorMap(depthColor, depthColor, cv::COLORMAP_JET);
    __depthItem->updateImage(QImage(depthColor.data, depthColor.cols,
                                    depthColor.rows, QImage::Format_RGB888)
                                 .copy());
    __textureItem->updateImage(
        QImage(__frameData.__textureMap.data, __frameData.__textureMap.cols,
               __frameData.__textureMap.rows, QImage::Format_RGB888));

    return isSucess;
}

bool SLCameraEngine::offLineCapture(const QString leftImgPath,
                                    const QString rightImgPath) {
...
bool SLCameraEngine::getStringAttribute(const QString attributeName,
                                        QString &val) {
...
bool SLCameraEngine::getNumbericalAttribute(const QString attributeName,
                                            double &val) {
...
bool SLCameraEngine::setStringAttribute(const QString attributeName,
                                        const QString val) {
    return __camera->setStringAttribute(attributeName.toStdString(),
                                        val.toStdString());
}

bool SLCameraEngine::setNumbericalAttribute(const QString attributeName,
                                            const double val) {
    return __camera->setNumberAttribute(attributeName.toStdString(), val);
}

bool SLCameraEngine::setBooleanAttribute(const QString attributeName,
                                         const bool val) {
...
bool SLCameraEngine::updateCamera() { return __camera->updateCamera(); }
...
void SLCameraEngine::bindDepthAndTextureItem(ImagePaintItem *depthItem,
                                             ImagePaintItem *textureItem) {
