...
void VTKProcessEngine::init(QQmlApplicationEngine *engine,
                            QString renderItemObjName,
                            QString statusBarItemObjName) {
  ...
  __axesActor->SetTotalLength(axesActor_length, axesActor_length,
                              axesActor_length);
  __axesActor->GetXAxisCaptionActor2D()
      ->GetTextActor()
      ->SetTextScaleModeToNone();
  __axesActor->GetYAxisCaptionActor2D()
      ->GetTextActor()
      ->SetTextScaleModeToNone();
  __axesActor->GetZAxisCaptionActor2D()
      ->GetTextActor()
      ->SetTextScaleModeToNone();
  __axesActor->GetXAxisCaptionActor2D()->GetCaptionTextProperty()->SetFontSize(
      axesActor_label_font_size);
  __axesActor->GetYAxisCaptionActor2D()->GetCaptionTextProperty()->SetFontSize(
      axesActor_label_font_size);
  __axesActor->GetZAxisCaptionActor2D()->GetCaptionTextProperty()->SetFontSize(
      axesActor_label_font_size);
  // Platform Grid
  ...
  for (int16_t i = -platformWidth / 2; i <= platformWidth / 2; i += gridSize) {
    createLine(i, -platformDepth / 2, gridBottomHeight, i, platformDepth / 2,
               gridBottomHeight, gridPoints, gridCells);
  }

  for (int16_t i = -platformDepth / 2; i <= platformDepth / 2; i += gridSize) {
    createLine(-platformWidth / 2, i, gridBottomHeight, platformWidth / 2, i,
               gridBottomHeight, gridPoints, gridCells);
  }
  ...
  __renderItem->renderer()->SetBackground(24.f / 255.f, 24.f / 255.f,
                                          24.f / 255.f);
  __renderItem->renderer()->SetBackground2(88.f / 255.f, 88.f / 255.f,
                                           88.f / 255.f);
  ...
  __renderItem->renderWindow()->renderWindow()->GetInteractor()->SetPicker(
      areaPicker);
  __renderItem->renderWindow()
      ->renderWindow()
      ->GetInteractor()
      ->SetInteractorStyle(style);

  vtkNew<vtkAxesActor> axesActor;
  axesActor->SetTotalLength(axesActor_length, axesActor_length,
                            axesActor_length);
  ...
  __orientationWidget->SetInteractor(
      __renderItem->renderWindow()->renderWindow()->GetInteractor());
  ...
  __renderItem->renderer()->GetActiveCamera()->SetPosition(
...
void VTKProcessEngine::renderCloud(
  ...
  for (size_t i = 0; i < cloud.points.size(); ++i) {
    vtkIdType pid[1];
    pid[0] = points->InsertNextPoint(cloud.points[i].x, cloud.points[i].y,
                                     cloud.points[i].z);
    lookup->SetTableValue(i, cloud.points[i].r / 255.f,
                          cloud.points[i].g / 255.f, cloud.points[i].b / 255.f,
                          1);
  ...
  __scalarBar->GetPositionCoordinate()
...
void VTKProcessEngine::saveCloud(const QString path) {
  ...
  __asyncThread = std::thread([&, path] {
    ...
    __statusLabelVal =
        "status: save point cloud finished, save path is[" + path + "]";
...
void VTKProcessEngine::clip(const bool isClipInner) {
  ...
  __asyncThread = std::thread([&, isClipInner] {
    ...
    std::thread clipThread =
        std::thread([&] { style->clip(isClipInner, progressVal); });
    ...
    vtkLookupTable::SafeDownCast(__scalarBar->GetLookupTable())
...
void VTKProcessEngine::cancelClip() { style->cancelClip(); }
...
void VTKProcessEngine::statisticalOutRemoval(const float stdThreshold,
                                             const float meanK) {
  if (!__cloud->GetMapper()) {
    return;
  }
  ...
  __asyncThread = std::thread([&, stdThreshold, meanK] {
    ...
    auto lookupTable =
        vtkLookupTable::SafeDownCast(__cloud->GetMapper()->GetLookupTable());
    ...
    for (size_t i = 0; i < indicesFiltered.size(); ++i) {
      vtkIdType pid[1];
      pid[0] =
          pointsVertex->InsertNextPoint(cloud->points[indicesFiltered[i]].x,
                                        cloud->points[indicesFiltered[i]].y,
                                        cloud->points[indicesFiltered[i]].z);
      lookupTable->SetTableValue(i, cloud->points[indicesFiltered[i]].r / 255.f,
                                 cloud->points[indicesFiltered[i]].g / 255.f,
                                 cloud->points[indicesFiltered[i]].b / 255.f,
                                 1);
      polyVertex->GetPointIds()->SetId(i, i);
      scalars->InsertValue(i, i);
    }
    ...
    vtkDataSetMapper::SafeDownCast(__cloud->GetMapper())
        ->SetScalarRange(0, indicesFiltered.size() - 1);

    vtkLookupTable::SafeDownCast(__scalarBar->GetLookupTable())
        ->SetTableRange(__cloud->GetZRange()[0], __cloud->GetZRange()[1]);
...
void VTKProcessEngine::surfaceRestruction() {
  ...
  __asyncThread = std::thread([&] {
    ...
    std::thread workThread = std::thread([&] {
      ...
      pcl::search::KdTree<pcl::PointNormal>::Ptr searchTreeNormals(
...
void VTKProcessEngine::jetDepthColorMap() {
  ...
  auto points =
      vtkUnstructuredGrid::SafeDownCast(__cloud->GetMapper()->GetInput())
          ->GetPoints();
  ...
  auto scalars = vtkFloatArray::SafeDownCast(
      vtkUnstructuredGrid::SafeDownCast(
          vtkDataSetMapper::SafeDownCast(__cloud->GetMapper())->GetInput())
          ->GetPointData()
          ->GetScalars());
...
void VTKProcessEngine::setCameraViewPort(double x, double y, double z,
  ...
  if (__cloud->GetMapper()) {
    ...
    if (x != 0) {
      ...
      x < 0 ? camera->SetFocalPoint(bounds[0] + x + 100, center[1], center[2])
            : camera->SetFocalPoint(bounds[1] + x - 100, center[1], center[2]);
    } else if (y != 0) {
      y < 0 ? camera->SetPosition(center[0], bounds[2] + y, center[2])
            : camera->SetPosition(center[0], bounds[3] + y, center[2]);
      y < 0 ? camera->SetFocalPoint(center[0], bounds[2] + y + 100, center[2])
            : camera->SetFocalPoint(center[0], bounds[3] + y - 100, center[2]);
    } else if (z != 0) {
      z < 0 ? camera->SetPosition(center[0], center[1], bounds[4] + z)
            : camera->SetPosition(center[0], center[1], bounds[5] + z);
      z < 0 ? camera->SetFocalPoint(center[0], center[1], bounds[4] + z + 100)
            : camera->SetFocalPoint(center[0], center[1], bounds[5] + z - 100);
    }
  } else {
