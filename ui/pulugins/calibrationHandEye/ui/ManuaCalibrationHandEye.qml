...
ApplicationWindow{
    ...
    Rectangle {
        ...
        Rectangle {
            ...
            Rectangle {
                ...
                Text {
                    ...
                    text: qsTr("Hand Eye Calibration Tool")
                    anchors.left: parent.left
                    anchors.top: parent.top
                    anchors.bottom: parent.bottom
                    anchors.leftMargin: 8
        ...
        CaliHandEyeLaunch {
            id: caliHandEyeLaunch
            objectName: "caliHandEyeLaunch"

            Component.onCompleted: {
                GlobalSignals.getIntrisic();
            }
        }
        ...
        Connections {
            ...
            function onConnectRobot(robotName, robotIp) {
                if(!caliHandEyeLaunch.connect(robotName, robotIp)) {
                    messageDialog.text = "connect robot failure.";
                    messageDialog.open();
                }
            }

            function onDisConnect() {
                if(!caliHandEyeLaunch.disConnect()) {
                    messageDialog.text = "dis connect robot failure.";
                    messageDialog.open();
                }
            }

            function onSucessImgPath(writePath) {
                if(!caliHandEyeLaunch.logPose(writePath)) {
                    messageDialog.text = "log pose failued!can't find corners.";
                    messageDialog.open();
                }
            }

            function onCalibrate() {
                var errorLists = caliHandEyeLaunch.calibrate();
                GlobalSignals.calibrateFinish(errorList);
            }

            function onSetTarget(targetType, params) {
                caliHandEyeLaunch.setTarget(targetType, params);
            }

            function onDiscard(index, filePath) {
                caliHandEyeLaunch.discard(index, filePath);
            }
        }
    }
}
